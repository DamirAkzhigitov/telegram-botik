 ...
 bot.on(message(), async (ctx) => {
  try {
   ...
   if (sessionData.promptNotSet) {
    ...
    return await ctx.telegram.sendMessage(chatId, 'Системный промт обновлен!')
   }
   ...
   let newMessageText: string = userMessage
   ...
   if ('photo' in ctx.message) {
    ...
    const photoUrl = await bot.telegram.getFileLink(file.file_path || '')

    newMessageText = `[Photo] - ${photoUrl}`
   }
   ...
   /**
    *  Replaced: userMessage -> newMessage.text
    *  @usage fixed the bug, where if user send photo message, then openAi send request with text message undefined
    *
    */
   const recentMessages = [...sessionData.userMessages].map((m) => `${m.name}[${m.time}]: ${m.text};`).reverse().join(';')
   ...
   const asyncActions = botMessages.map(async ({ content, type }) => {
    if (type === 'emoji') {
     ...
     const stickerByEmoji = findByEmoji(response.stickers as Sticker[],
      content,
     )
